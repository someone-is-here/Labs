...
namespace Lab3 {
    class Program {
        static void Main(string[] args) {
            ...
            hp.hitHandler += (int newAccuracy) => Console.WriteLine($"Changed hit accuracy from {hp.HitAccuracy} to {newAccuracy}");
            int j = 5;
            while (--j > 0) {
                hp.Attack();
            }
            ...
            vp.hitHandler += (int newAccuracy) => Console.WriteLine($"Changed hit accuracy from {vp.HitAccuracy} to {newAccuracy}");
            vp.Attack();
